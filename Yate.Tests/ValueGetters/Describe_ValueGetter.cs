...
namespace Yate.Tests.ValueGetters
{
    //taken from https://github.com/jdiamond/Nustache/blob/master/Nustache.Core.Tests/Describe_ValueGetter.cs
    [TestFixture]
    public class Describe_ValueGetter
    {
        ...
        [Test]
        public void It_gets_Dictionary_values_WhenColumnDoesNotExist()
        {
            Dictionary<string, int> target = new Dictionary<string, int>();
            target["IntKey"] = 123;
            Assert.AreEqual(NoValueGetter.NoValue, ValueGetter.GetValue(target, "cbvgf"));
        }
        ...
        [Test]
        public void It_gets_case_insensitive_DataRowView_values_whenColumnDoesNotExist()
        {
            DataTable dt = new DataTable();
            dt.Columns.Add("IntColumn", typeof(int));
            dt.Rows.Add(new object[] { 123 });
            DataRowView target = dt.DefaultView[0];
            Assert.AreEqual(NoValueGetter.NoValue, ValueGetter.GetValue(target, "intcoasdflumn"));
        }
        ...
        public void ElementDoesNotExist()
        {
            ...
            target.LoadXml("<doc attr='val'><child>text</child></doc>");
            var elements = ValueGetter.GetValue(target.DocumentElement, "DoesNotExist");
            Assert.AreEqual(ValueGetter.NoValue, elements);
        }
        ...
        public void AttributeDoesNotExist()
        {
            var target = new XmlDocument();
            target.LoadXml("<doc attr='val'><child>text</child></doc>");
            var elements = ValueGetter.GetValue(target.DocumentElement, "@asdf");
            Assert.AreEqual(ValueGetter.NoValue, elements);
        }
